...
import sys
import os
sys.path.append(os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'Command Center', 'Data Bus', 'Bus Core Design'))
from universal_communicator import UniversalCommunicator

# Import OCR Flow Engine for automatic evidence processing
try:
    ocr_flow_path = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "The War Room", "SOPs", "READ FILES", "Build Specs")
    if ocr_flow_path not in sys.path:
        sys.path.insert(0, ocr_flow_path)
    from ocr_flow_engine import OCRFlowEngine
    OCR_FLOW_ENGINE_AVAILABLE = True
except ImportError as e:
    OCRFlowEngine = None
    OCR_FLOW_ENGINE_AVAILABLE = False
    print(f"WARNING: OCRFlowEngine not available: {e}")
...
class EvidenceLocker:
    ...
    def __init__(self, ecc=None, gateway=None, bus=None):
        ...
        if bus:
            import logging
            logger = logging.getLogger("EvidenceLocker")
            logger.info("[Evidence Locker] Waiting for bus stabilization...")
            if not bus.wait_for_ready(timeout=15.0):
                logger.error("[Evidence Locker] Bus stabilization timeout - initialization may be unstable")
                return
            
            logger.info("[Evidence Locker] Bus ready - proceeding with initialization...")
            # NOTE: Module initializes immediately, no waiting for turn
            # The bus sequence is for independent processes, not inline initialization
        ...
        if self.bus:
            import logging
            logger = logging.getLogger("EvidenceLocker")
            if self.bus.register_module_init('1', {
                'version': '1.0',
                'type': 'evidence_locker',
                'capabilities': ['evidence_management', 'evidence_processing', 'evidence_storage']
            }):
                logger.info("[Evidence Locker] [OK] Module registered with bus (Address 1)")
            else:
                logger.warning("[Evidence Locker] Module registration failed - continuing anyway")
        ...
        # Initialize OCR Flow Engine for automatic evidence processing
        if OCR_FLOW_ENGINE_AVAILABLE:
            try:
                self.ocr_engine = OCRFlowEngine()
                self.logger.info("[1] OCR Flow Engine initialized - MANDATORY processing enabled")
            except Exception as e:
                self.logger.error(f"[1] CRITICAL: OCR Flow Engine initialization failed: {e}")
                self.ocr_engine = None
        else:
            self.logger.error("[1] CRITICAL: OCR Flow Engine NOT AVAILABLE - Evidence processing degraded")
            self.ocr_engine = None
    ...
    def _handle_communication_signal(self, payload: Dict[str, Any]) -> None:
        ...
        self.logger.info(f"[3-1] Communication signal received from {caller_address}: {radio_code}")
        
        # Build response payload
        response = {
            'responder_address': '3-1',
            'responder_name': 'Evidence_Locker',
            'signal_id': signal_id,
            'timestamp': datetime.now().isoformat()
        }
        
        if radio_code == 'ROLLCALL':
            # Report operational status and evidence count
            with self._manifest_lock:
                evidence_count = len(self.evidence_index)
            
            response['status'] = '10-4'  # Operational
            response['subsystems'] = {
                'evidence_index': 'operational',
                'manifest_system': 'operational',
                'total_evidence': evidence_count
            }
            self.logger.info(f"[3-1] ROLLCALL response sent - {evidence_count} evidence items indexed")
            
        elif radio_code in ['STATUS', 'RADIO_CHECK']:
            response['status'] = '10-4'  # Available
            self.logger.info(f"[3-1] {radio_code} acknowledged")
    ...
    def initialize_with_bus(self, bus):
        ...
        try:
            ...
            bus.register_signal("communication", self._handle_communication_signal)
    ...
    def build_manifest(self, output_path, case_id=None):
        ...
        manifest = {
            ...
            "system_status": {
                "ocr_engine": {
                    "initialized": bool(self.ocr_engine),
                    "engines_available": getattr(self.ocr_engine, 'engines_available', {}),
                    "fallback_enabled": bool(getattr(getattr(self.ocr_engine, 'ocr_config', {}), 'get', lambda *a, **k: False)('fallback_enabled', False)),
                },
    ...
    def _infer_ocr_file_type(self, file_ext: str) -> str:
        file_ext = file_ext.lower()
        if file_ext in {'.png', '.jpg', '.jpeg', '.tiff', '.bmp', '.gif', '.heic', '.heif'}:
            return 'image'
        if file_ext == '.pdf':
            return 'pdf'
        if file_ext in {'.docx', '.doc'}:
            return 'docx'
        if file_ext in {'.txt', '.rtf', '.md'}:
            return 'text'
    ...
    def process_evidence_comprehensive(self, file_path: str) -> Dict[str, Any]:
        """Process evidence using OCR Flow Engine cascades plus secondary analyzers."""
        try:
            if not self.ocr_engine:
                raise RuntimeError('OCR Flow Engine is not initialized; evidence processing cannot continue.')

            file_ext = os.path.splitext(file_path)[1].lower()
            ocr_file_type = self._infer_ocr_file_type(file_ext)
            timestamp = datetime.now().isoformat()
            ...
            ocr_output = self.ocr_engine.process_file(file_path, ocr_file_type)
            processing_result['ocr'] = ocr_output
            processing_result['ocr_confidence'] = ocr_output.get('confidence', 0.0)
            processing_result['ocr_engine_used'] = ocr_output.get('engine_used')
            processing_result['ocr_fallback_attempts'] = list(ocr_output.get('fallback_attempts', []))
            processing_result['metadata'] = ocr_output.get('metadata', {})
            processing_result['ai_notes'] = list(ocr_output.get('ai_notes', []))
            processing_result['tables'] = list(ocr_output.get('tables', []))
            processing_result['entities'] = list(ocr_output.get('entities', []))

            text_blocks = ocr_output.get('text_blocks') or []
            processing_result['ocr_text'] = '\n'.join(
                block.get('text', '') for block in text_blocks if isinstance(block, dict)
            )

            engine_used = processing_result['ocr_engine_used'] or 'unknown'
            processing_result['tools_used'].append(f'ocr:{engine_used}')
            for attempt in processing_result['ocr_fallback_attempts']:
                processing_result['tools_used'].append(f'ocr_fallback:{attempt}')

            if ocr_file_type == 'video':
                video_analysis = self.extract_frames_from_video(file_path)
                processing_result['video_analysis'] = video_analysis
                processing_result['tools_used'].append('video:moviepy')
            ...
            if file_ext in ['.pdf', '.docx', '.doc', '.txt']:
                ...
                if document_structure:
                    processing_result['tools_used'].append('doc:structure')
            ...
            log_entry = {
                ...
                'ocr_engine_used': processing_result['ocr_engine_used'],
                'ocr_fallback_attempts': processing_result['ocr_fallback_attempts'],
            }
            self.processing_log.append(log_entry)

            self.logger.info('[EVIDENCE] OCR Flow processed %s using %s', file_path, engine_used)
            return processing_result

        except Exception as exc:
            self.logger.error(f'Comprehensive processing failed for {file_path}: {exc}')
