...
class EvidenceManager:
    ...
    def _merge_nested(self, base: Dict[str, Any], updates: Dict[str, Any]) -> Dict[str, Any]:
        ...
        """Deep-merge helper so AI case updates can extend existing metadata."""
        ...
        for key, value in updates.items():
            ...
            if key in base and isinstance(base[key], dict) and isinstance(value, dict):
                ...
                base[key] = self._merge_nested(base[key], value)
    ...
    def _enrich_with_ai(self, evidence_record: Dict[str, Any]) -> Optional[Dict[str, Any]]:



        """Run AI enrichment via the orchestrator when available."""
        ...
        try:
            ...
            ai_payload = self.ai_orchestrator.enrich_evidence(evidence_record)
            ...
            self.logger.exception("AI orchestrator failed for %s: %s", evidence_record.get("filename", "evidence"), exc)
        ...
        case_updates = ai_payload.get("case_updates")
        ...
        if isinstance(case_updates, dict) and case_updates:
            ...
            existing = metadata.get("ai_case_updates", {})
            ...
            if isinstance(existing, dict):
                ...
                metadata["ai_case_updates"] = self._merge_nested(dict(existing), case_updates)
                ...
                metadata["ai_case_updates"] = case_updates



        if ai_payload.get('status') != 'ok':
            self.logger.warning("AI enrichment result for %s: %s", evidence_record.get('filename', 'evidence'), ai_payload.get('reason') or ai_payload.get('error', 'unknown issue'))
    ...
    def process_evidence(self, evidence_id: str) -> bool:
        ...
        try:
            ...
            if is_valid:
                ...
                ai_payload = self._enrich_with_ai(evidence_record)



                if ai_payload:
                    evidence_record['ai_analysis'] = ai_payload
                    evidence_record.setdefault('processing_log', []).append({
                        'timestamp': datetime.now().isoformat(),
                        'action': 'ai_enrichment',
                        'provider': ai_payload.get('provider'),
                        'status': ai_payload.get('status'),
                        'message': ai_payload.get('reason') or ai_payload.get('error'),
                    })
