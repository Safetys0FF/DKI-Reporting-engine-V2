# Section 6 Rebuild Playbook (Draft)

## 1. Mission & Theory of Operation
Translate operational activity into a defensible financial summary across report modes.

## 2. Functional Responsibilities
- Primary deliverable: Billing Summary output aligned with case governance.
- Evidence sources: contract totals, prep costs, mileage, time logs, subcontractor expenses.
...
- Processing stages:
  ...
  3. Normalize results into `section_6_payload` with `render_tree`, `manifest`, and validation flags.
...
- Core tooling: mileage_tool_v_2, northstar_protocol_tool, billing_tool_engine, repository metadata.
...
- Inputs pulled via `gateway.get_section_inputs('section_6')`.
- Outputs pushed through `gateway.publish_section_result('section_6', payload)`.
- Shared data contracts:
  - Provide digestible manifests for downstream sections (Depends on Sections 1-4 for scope proof; outputs feed final assembly and client finance systems; cross-verified with Section 9 disclosures.).
  - Subscribe to status signals from related sections to handle sequencing without race conditions.

## 8. Operational SOP
1. Gateway marks prerequisites satisfied and issues `prepare_section_6` command.
...
- [ ] Update schemas (`section_6_payload`, manifests) to include confirmations, fallbacks, and provenance.
