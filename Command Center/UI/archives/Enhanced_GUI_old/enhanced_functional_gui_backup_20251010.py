...
class LoginDialog:
    ...
    def __init__(
        ...
        *,
        ...
        initial_role = persisted_login.get("last_role", "field_operator")
        ...
        self.role_var = tk.StringVar(value=initial_role or "field_operator")
        ttk.Combobox(
            new_frame,
        ...
        if self.operators and persisted_login.get("last_operator"):
            for idx, profile in enumerate(self.operators):
                if profile.name == persisted_login["last_operator"]:
                    try:
    ...
    def _accept(self) -> None:
        if self.manager:
            if self.selected_profile is not None:
                ...
                self._persist_selection(self.selected_profile.name, getattr(self.selected_profile, "role", None))
                self.window.destroy()
                return
            ...
            if not name:
                ...
                return
            self.result = name
            self._persist_selection(name, self.role_var.get())
        self.window.destroy()
        if self.callback:
...
class ProfileEditor:
    ...
    def __init__(self, parent: tk.Tk, profile_data: Dict[str, object], callback: Optional[Callable[[Optional[Dict[str, object]]], None]] = None) -> None:
        self.callback = callback
        self.window = tk.Toplevel(parent)
...
class EnhancedDKIGUI:
    ...
    def __init__(self, bus=None) -> None:
        ...
        if bus:
            try:
                # Add Data Bus to path for UniversalCommunicator
                import sys
                import os
                data_bus_path = str(Path(__file__).resolve().parents[2] / "Command Center" / "Data Bus")
                ...
                self.logger.critical(f"[GUI-1] CANBUS connection failed: {e}")
        ...
        self.operator_name: str = login_defaults.get("last_operator") or self.profile.display_name
    ...
    def _show_profile_manager(self) -> None:
        ...
        ProfileEditor(self.root, raw_profile, callback=self._on_profile_saved)
    ...
    def _build_home_tab(self, parent: ttk.Frame) -> None:
        ...
        operator_panel.columnconfigure(0, weight=1)
        ttk.Label(
            operator_panel,
    ...
    def _show_login_dialog(self, initial: bool = False) -> None:
        ...
        result = dialog.show()
        selected_role = dialog.role_var.get() if hasattr(dialog, "role_var") else None
        if isinstance(result, OperatorProfile):
            self.operator_profile = result
            self._renew_operator_token(result)
            self.operator_role_default = result.role or selected_role or self.operator_role_default
            ...
            self.operator_token_payload = None
            self.operator_role_default = selected_role or self.operator_role_default
            ...
            self.operator_profile = None
            self.operator_token_payload = None
            self._set_operator(fallback, self.operator_role_default)
    ...
    def _initialize_plugin(self) -> None:
        try:
            ...
            self.categories = []  # Will be populated from bus signals
    ...
    def _advertise_card_need(self, card: EvidenceCard, slug: str, extra_tags: Iterable[str]) -> None:
        ...
        try:
            ...
            result = self.section_adapter.publish_category_need(slug, **kwargs) if self.section_adapter else {}
    ...
    def _scan_card(self, card: EvidenceCard, slug: str, extra_tags: Iterable[str]) -> None:
        ...
        try:
            # Emit evidence scan signal
            self.bus.emit("evidence.scan", payload)
            response = {"status": "queued"}  # Async - will get response via signal
            case_context = card.case_label or case_id or "unassigned"
            self._append_log(
    ...
    def _refresh_report_summary(self) -> None:
        ...
        try:
            ...
            self.bus.emit("report.generate", {"case_id": self.current_case_id})
    ...
    def _export_report(self) -> None:
        ...
        try:
            # Emit export signal
            self.bus.emit("report.export", {"report": self.current_report, "file_path": target_path})
...
def main(argv: Sequence[str] | None = None) -> int:
    try:
        ...
        bus = None
        try:
            data_bus_path = str(Path(__file__).resolve().parents[2] / "Command Center" / "Data Bus" / "Bus Core Design")
            if data_bus_path not in sys.path:
                sys.path.append(data_bus_path)
            from bus_core import DKIReportBus
            bus = DKIReportBus()
