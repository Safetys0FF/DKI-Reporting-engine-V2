...
class LoginDialog:
    """Prompt for operator name when launching or switching profiles."""

    def __init__(self, parent: tk.Tk, default_name: str) -> None:
        ...
        ttk.Label(self.window, text="Operator Name:").grid(row=0, column=0, padx=12, pady=(12, 4), sticky="w")
        self.name_var = tk.StringVar(value=default_name or "Analyst")
        ttk.Entry(self.window, textvariable=self.name_var, width=32).grid(row=1, column=0, padx=12, sticky="ew")

        button_row = ttk.Frame(self.window)
        button_row.grid(row=2, column=0, padx=12, pady=(10, 12), sticky="e")
        ttk.Button(button_row, text="Cancel", command=self._cancel).pack(side="right", padx=(6, 0))
        ttk.Button(button_row, text="Continue", command=self._accept).pack(side="right")
...
class EnhancedDKIGUI:
    ...
    def _build_layout(self) -> None:
        ...
        self.title_label = tk.Label(header, text=APP_TITLE, font=("Segoe UI", 18, "bold"))
        self.title_label.grid(row=0, column=0, sticky="w")
        self.operator_label = tk.Label(header, text="", font=("Segoe UI", 10), fg="#4b5563")
        self.operator_label.grid(row=1, column=0, sticky="w", pady=(4, 0))
        ...
        self.nav_panel = tk.Frame(main_area, bg="#111827", width=220)
        self.nav_panel.grid(row=0, column=0, sticky="ns")
        self.nav_panel.grid_propagate(False)

        self.space_container = ttk.Frame(main_area)
        self.space_container.grid(row=0, column=1, sticky="nsew")
        self.space_container.columnconfigure(0, weight=1)
        self.space_container.rowconfigure(0, weight=1)

        self._build_touch_nav(self.nav_panel)

        home_space = ttk.Frame(self.space_container, padding=24)
        cases_space = ttk.Frame(self.space_container, padding=24)
        workspace_space = ttk.Frame(self.space_container, padding=18)
        review_space = ttk.Frame(self.space_container, padding=24)
        assembly_space = ttk.Frame(self.space_container, padding=24)

        self.spaces = {
            "home": home_space,
            "cases": cases_space,
            "workspace": workspace_space,
            "review": review_space,
            "assembly": assembly_space,
        }

        self._build_home_tab(home_space)
        self._build_cases_tab(cases_space)
        self._build_workspace_tab(workspace_space)
        self._build_review_tab(review_space)
        self._build_assembly_tab(assembly_space)

        for frame in self.spaces.values():
        ...
        self.status_bar.add_section("status", "Initializing")
        self.status_bar.add_section("bus", "Bus: pending")
        self.status_bar.add_section("cards", "Evidence cards: 0")
        self.status_bar.add_section("profile", "")
    ...
    def _build_touch_nav(self, parent: tk.Widget) -> None:
        ...
        style.configure("Nav.TButton", padding=(24, 16), font=("Segoe UI", 12, "bold"), anchor="w")

        title = tk.Label(parent, text=APP_TITLE, bg="#111827", fg="#f8fafc", font=("Segoe UI", 14, "bold"), wraplength=180, justify="left")
        title.pack(fill="x", padx=16, pady=(24, 12))
        ...
        for key, label in nav_items:
            button = ttk.Button(parent, text=label, style="Nav.TButton", command=lambda k=key: self._select_tab(k))
            button.pack(fill="x", padx=16, pady=6)
            self.nav_buttons[key] = button

        tk.Frame(parent, bg="#111827").pack(expand=True, fill="both")
    ...
    def _build_home_tab(self, parent: ttk.Frame) -> None:
        profile_frame = ttk.LabelFrame(parent, text="Analyst Profile", padding=12)
        profile_frame.pack(fill="x")
        ttk.Label(profile_frame, textvariable=self.home_profile_text, justify="left").pack(anchor="w")

        case_frame = ttk.LabelFrame(parent, text="Case Defaults", padding=12)
        case_frame.pack(fill="x", pady=(12, 0))
        ttk.Label(case_frame, textvariable=self.home_case_text, justify="left").pack(anchor="w")

        actions = ttk.Frame(parent, padding=(0, 18))
        actions.pack(fill="x", pady=(12, 0))
        ttk.Button(actions, text="Open Workspace", command=self._open_workspace_tab).pack(side="left", padx=(0, 10))
        ttk.Button(actions, text="Refresh Profile", command=self._refresh_home_overview).pack(side="left")
...
".join(lines).strip()
    ...
    def _set_operator(self, name: str) -> None:
        ...
        self.operator_label.configure(text=f"Operator: {name}")
        if self.status_bar:
            self.status_bar.update_section("profile", f"Operator: {name}")

    def _show_login_dialog(self, initial: bool = False) -> None:
        dialog = LoginDialog(self.root, self.operator_name)
        result = dialog.show()
        if result:
            ...
            self._set_operator(self.profile.display_name or "Analyst")
    ...
    def _refresh_home_overview(self) -> None:
        ...
        case_lines = [
            ...
            f"Default Contract: {contracts.get('default', 'n/a')}",
        ]
    ...
    def _update_status(self, message: str, *, bus_state: Optional[str] = None) -> None:
        if self.status_bar:
            ...
            if bus_state is not None:
                self.status_bar.update_section("bus", f"Bus: {bus_state}")
        self._sync_card_count()
