...
def main():
    ...
    if system_running:
        logger.info("✓ NORMAL BOOT MODE: System detected - joining existing operations")
        logger.info("  Bus active with connected modules")
        logger.info("  UDS will monitor and register modules as they come online")
    else:
        logger.info("⚠ SAFE MODE: No system detected - initializing critical modules")
        logger.info("  Starting complete system (11 modules) for remote maintenance/repair")
        logger.info("=" * 80)
        
        safe_mode_success = _initialize_safe_mode_modules(logger)
        
        if safe_mode_success:
            logger.info("✓ Safe mode initialization SUCCESS - modules running")
...
def _detect_system_state(logger):
    ...
    DEESCALATION Agent: Fixed to verify systems are ALIVE, not just registered
    
    Detection logic:
    1. Check if bus core is accessible and has active registrations
    2. PING systems to verify they're actually responding
    3. Return True if system running, False if safe mode needed
    ...
    try:
        ...
        # Attempt to import and check bus
        try:
            sys.path.insert(0, str(Path(__file__).parent.parent.parent / "Bus Core Design"))
            from bus_core import DKIReportBus
            
            # Try to connect to existing bus
            test_bus = DKIReportBus()
            
            # Check if bus has any registered systems
            if hasattr(test_bus, 'system_registry') and test_bus.system_registry:
                system_count = len(test_bus.system_registry)
                logger.info(f"  Found {system_count} registered systems on bus")
                
                # CRITICAL FIX: Don't trust old registry - PING to verify they're alive
                logger.info("  VERIFYING systems are ALIVE (not just registered)...")
                
                # Try to send a quick ping signal to verify any system responds
                response_received = False
                response_timeout = 3  # Give 3 seconds total for anyone to respond
                
                def check_for_response(sig_data):
                    nonlocal response_received
                    response_received = True
                    logger.info(f"    ✓ Response received from {sig_data.get('source_address', 'UNKNOWN')}")
                ...
                try:
                    ...
                    if response_received:
                        logger.info("  ✓ SYSTEM ALIVE: At least one module responded")
                        return True
                    else:
                        logger.warning("  ✗ SYSTEM DEAD: No responses from any registered systems")
                        logger.warning("  Registry has {0} systems but NONE are running".format(system_count))
                        logger.warning("  SAFE MODE required to start fresh system")
                        return False
                ...
                logger.info("  Bus exists but no systems registered")
                return False
                
        except Exception as bus_error:
            logger.info(f"  Bus not accessible: {bus_error}")
            return False
...
def _initialize_safe_mode_modules(logger):
    ...
    DEESCALATION Agent: Safe mode now starts FULL system for diagnostics:
    - Bus Core (CANBUS/LINBUS)
    ...
    This matches the phased initialization orchestrator in main_application.py.
    ...
    try:
        ...
        logger.info("=" * 80)
        logger.info("SAFE MODE: FULL SYSTEM INITIALIZATION FOR CODE TESTING")
        logger.info("=" * 80)
        
        # PHASE 1: Bus Core (CANBUS/LINBUS)
        logger.info("PHASE 1: Initializing bus core (CANBUS/LINBUS)...")
        try:
            sys.path.insert(0, str(Path(__file__).parent.parent.parent / "Bus Core Design"))
            from bus_core import DKIReportBus
            bus = DKIReportBus()
            logger.info("✓ Bus core initialized")
            time.sleep(2)  # Stabilization delay
        except Exception as e:
            logger.error(f"✗ Bus core initialization FAILED: {e}")
            return False
        
        # PHASE 2: Warden (ECC + Gateway)
        logger.info("PHASE 2: Initializing Warden (ECC + Gateway Controller)...")
        try:
            sys.path.insert(0, str(Path(__file__).parent.parent.parent.parent.parent / "The Warden"))
            from warden_module import WardenModule
            
            def start_warden():
                warden = WardenModule(bus=bus, communicator=None)
                logger.info("✓ Warden module initialized (Address 2-1)")
            
            warden_thread = threading.Thread(target=start_warden, daemon=True)
            warden_thread.start()
            time.sleep(2)  # Stabilization delay
        except Exception as e:
            logger.warning(f"⚠ Warden initialization failed: {e}")
        
        # PHASE 3: Evidence Locker
        logger.info("PHASE 3: Initializing Evidence Locker...")
        try:
            ...
            def start_evidence_locker():
                evidence_locker = EvidenceLocker(bus=bus, ecc=None)
                logger.info("✓ Evidence Locker initialized (Address 3-1)")
            
            locker_thread = threading.Thread(target=start_evidence_locker, daemon=True)
            locker_thread.start()
            time.sleep(2)  # Stabilization delay
        except Exception as e:
            logger.warning(f"⚠ Evidence Locker initialization failed: {e}")
        
        # PHASE 4: Marshall + Evidence Manager
        logger.info("PHASE 4: Initializing Marshall + Evidence Manager...")
        try:
            ...
            def start_marshall():
                marshall = MarshallModule(bus=bus, communicator=None)
                evidence_mgr = EvidenceManager()
                marshall.attach_evidence_manager(evidence_mgr)
                logger.info("✓ Marshall + Evidence Manager initialized (Address 3)")
            
            marshall_thread = threading.Thread(target=start_marshall, daemon=True)
            marshall_thread.start()
            time.sleep(2)  # Stabilization delay
        except Exception as e:
            logger.warning(f"⚠ Marshall initialization failed: {e}")
        
        # PHASE 6: All 8 Analyst Sections
        logger.info("PHASE 6: Initializing all 8 Analyst Sections (4-1 to 4-8)...")
        try:
            ...
            def start_section1():
                section1 = Section1Framework(bus=bus)
                logger.info("✓ Section 1 initialized")
            
            section1_thread = threading.Thread(target=start_section1, daemon=True)
            section1_thread.start()
            time.sleep(0.5)
        except Exception as e:
            logger.warning(f"⚠ Section 1 initialization failed: {e}")
        ...
        try:
            ...
            def start_section2():
                section2 = LegacySection2Framework(bus=bus)
                logger.info("✓ Section 2 initialized")
            
            section2_thread = threading.Thread(target=start_section2, daemon=True)
            section2_thread.start()
            time.sleep(0.5)
        except Exception as e:
            logger.warning(f"⚠ Section 2 initialization failed: {e}")
            ...
            try:
                ...
                if section_class:
                    def start_section(num=section_num, cls=section_class):
                        section = cls(bus=bus)
                        logger.info(f"✓ Section {num} initialized")
                    
                    thread = threading.Thread(target=start_section, daemon=True)
                    thread.start()
                    time.sleep(0.5)
                else:
                    logger.warning(f"⚠ Section {section_num}: Framework class not found")
                    
            except Exception as e:
                logger.warning(f"⚠ Section {section_num} initialization failed: {e}")
        ...
        try:
            ...
            def start_debrief():
                debrief = MissionDebriefModule(bus=bus, communicator=None, ecc=None, gateway=None)
                logger.info("✓ Mission Debrief + Narrative Assembler initialized (Address 5)")
            ...
            logger.warning(f"⚠ Mission Debrief initialization failed: {e}")
        ...
        logger.info("  2. Warden (2-1) - ECC + Gateway")
        logger.info("  3. Evidence Locker (3-1) - Evidence management")
    ...
    try:
        ...
        if system_running:
            ...
            if safe_mode_success:
                ...
                logger.warning("⚠ Safe mode initialization had issues")
...
"""
    ...
    def _check_auto_registration_compliance(
        ...
        try:
            ...
            time.sleep(0.5 if smoke_mode else 2)
    ...
    def _select_system_addresses(self, smoke_mode: bool) -> List[str]:
        ...
        priority_order = [
            "DIAG-1",
            "2-1",  # Warden / Ecosystem Controller
    ...
    def _instantiate_parent_modules(self):
        ...
        try:
            ...
            try:
                ...
                self.logger.info("[SMOKE MODE] ✓ Evidence Locker instantiated")
            except Exception as e:
            ...
            try:
                ...
                self.logger.info("[SMOKE MODE] ✓ Warden instantiated")
            except Exception as e:
            ...
            try:
                ...
                self.logger.info("[SMOKE MODE] ✓ Mission Debrief instantiated")
            except Exception as e:
    ...
    def _force_system_startup(self, smoke_mode: bool = False) -> bool:
        ...
        try:
            ...
            # SMOKE MODE FIX: Instantiate parent modules so they actually listen
            if smoke_mode:
    ...
    def _perform_baseline_testing(self, smoke_mode: bool = False) -> bool:
        """
        Perform baseline monitoring by waiting for parent module self-test faults.
        Parent modules run self-tests on startup and emit fault codes if children fail.
        UDS passively collects faults during monitoring period.
        
        This replaces the old 195-point active testing approach.
        """
        try:
            monitoring_period = 5 if smoke_mode else 15
            self.logger.info(f"[UDS] Baseline monitoring: Waiting {monitoring_period} seconds for parent module self-test faults...")
            ...
            # Wait for parent modules to complete self-tests and emit faults
